 ...
 * Contributors: https://github.com/107-systems/l3xz-pressure-sensor-firmware/graphs/contributors.
 */
...
static int const PRESSURE_0_INPUT_PIN = 27; /* ANALOG_INPUT 0 */
static int const PRESSURE_1_INPUT_PIN = 28; /* ANALOG_INPUT 1 */
...
cyphal::Publisher<uavcan::si::unit::pressure::Scalar_1_0> pressure_0_pub, pressure_1_pub;
...
const auto reg_ro_cyphal_node_description    = node_registry->route ("cyphal.node.description", {true}, []() { return "L3X-Z Hydraulic Pressure Sensor"; });
...
void setup()
{
  ...
  DBG_INFO("Node ID: %d\n\r\tPRESSURE 0 ID = %d\n\r\tPRESSURE 1 ID = %d",
           node_id, port_id_pressure_0, port_id_pressure_1);
  ...
  (
    ...
    "107-systems.l3xz-pressure-sensor"
  );
...
# error "Check ADC conversion for your platform or you may get erroneous results."
  ...
  if((now - prev_pressure_0) > UPDATE_PERIOD_PRESSURE_0_ms)
  {
    prev_pressure_0 = now;

    uint16_t const pressure_0_adc_in = analogRead(PRESSURE_0_INPUT_PIN);

    uavcan::si::unit::pressure::Scalar_1_0 msg;
    msg.pascal = analogInputToPascal(pressure_0_adc_in);

    if (pressure_0_pub)
      pressure_0_pub->publish(msg);
  }

  if((now - prev_pressure_1) > UPDATE_PERIOD_PRESSURE_1_ms)
  {
    prev_pressure_1 = now;

    uint16_t const pressure_1_adc_in = analogRead(PRESSURE_1_INPUT_PIN);

    uavcan::si::unit::pressure::Scalar_1_0 msg;
    msg.pascal = analogInputToPascal(pressure_1_adc_in);
